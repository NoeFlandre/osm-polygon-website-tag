...
def _is_unfinished_source_shard(state: RunState, path: Path) -> bool:
    ...
    return is_unfinished_source_shard(
        state, path, label="sentence", needs_work=shard_needs_sentence_segmentation
    )
...
_prepare_run_state = partial(prepare_stage_run_state, action="add sentences to")
...
def _write_sync_history(
    ...
    write_sync_history(
        run_dir / _MANIFESTS_DIRECTORY / _HISTORY_DIRECTORY,
        Path(bundle.shards[0].name).stem,
        bundle.payload(),
        result.payload(),
        completed=result.completed,
    )
