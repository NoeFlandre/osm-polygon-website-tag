...
def prepare_language_bundle(
    ...
    *,
    ...
    try:
        ...
        stage_model_file(model_path, target / model.filename)
...
def run_language_bundle(
    ...
    *,
    ...
    validate_grid_options(effective_budget, effective_batch_rows)
...
def _sync_paused_checkpoint(
    ...
    replace_directory(checkpoint.directory, language_checkpoint_store().directory_for(local))
...
def _load_bundle(directory: Path) -> Grid5000Bundle:
    ...
    payload = read_object(directory / BUNDLE_MANIFEST_NAME, "bundle")
    bundle = _bundle_from_payload(payload)
    safe_filename(bundle.source_shard)
...
def _bundle_from_payload(payload: Mapping[str, object]) -> Grid5000Bundle:
    ...
    bundle = Grid5000Bundle(
        ...
        source_shard=safe_filename(required_string(payload, "source_shard")),
        source_row_count=nonnegative_int(payload, "source_row_count"),
        source_shard_sha256=sha256_value(payload, "source_shard_sha256"),
    ...
    validate_grid_options(bundle.time_budget_seconds, bundle.batch_rows)
    return bundle
...
def _result_from_payload(payload: Mapping[str, object]) -> Grid5000Result:
    ...
    result = Grid5000Result(
        ...
        source_shard=safe_filename(required_string(payload, "source_shard")),
        source_row_count=nonnegative_int(payload, "source_row_count"),
        shard_sha256=sha256_value(payload, "shard_sha256"),
        model=model_from_payload(payload.get("model")),
        commit=required_string(payload, "commit"),
        completed=required_bool(payload, "completed"),
        changed=required_bool(payload, "changed"),
        processed_rows=nonnegative_int(payload, "processed_rows"),
        max_batch_rows=nonnegative_int(payload, "max_batch_rows"),
