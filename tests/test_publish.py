...
from osm_polygon_website_tag.publish import (
    PublishPlan,
    _upload_folder,
    build_publish_plan,
    create_repo,
...
def test_create_repo_requires_token(monkeypatch) -> None:
    ...
    monkeypatch.setattr("osm_polygon_website_tag.publish.resolve_hf_token", lambda: None)
    with pytest.raises(ValueError):
...
def test_create_repo_calls_hf_api(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "osm_polygon_website_tag.publish._create_repo_remote", fake_create_repo_remote
    )
    monkeypatch.setattr("osm_polygon_website_tag.publish.resolve_hf_token", lambda: "abc")
...
def test_create_repo_remote_requests_public_dataset(monkeypatch) -> None:
    from osm_polygon_website_tag.publish import _create_repo_remote
