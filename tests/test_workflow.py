...
from osm_polygon_website_tag.workflow import _upload_public_shard, discover_sources, run_all
...
def _inject_static_text_enrichment(monkeypatch: pytest.MonkeyPatch) -> None:
    from osm_polygon_website_tag import workflow
    from osm_polygon_website_tag.enrich import enrich_polygon_shard as real_enrich
...
def test_run_all_dry_run_completes_without_remote_calls(
    ...
    root = _sources(make_pbf, tmp_path)
    monkeypatch.setattr(
        "osm_polygon_website_tag.workflow._upload_public_shard",
        lambda *_args: pytest.fail("dry-run must not upload"),
    )
    monkeypatch.setattr(
        "osm_polygon_website_tag.workflow.publish_to_hf",
...
def test_run_all_apply_uploads_each_shard_then_complete_run(
    ...
    monkeypatch.setattr("osm_polygon_website_tag.workflow.resolve_hf_token", lambda: "available")
    monkeypatch.setattr(
        "osm_polygon_website_tag.workflow._upload_public_shard",
...
def test_complete_legacy_run_migrates_without_reextracting_pbf(
    ...
    monkeypatch.setattr(
        "osm_polygon_website_tag.workflow.extract_pbf",
...
def test_incremental_upload_includes_shard_and_recomputed_card(
    ...
    monkeypatch.setattr(
        "osm_polygon_website_tag.workflow._upload_folder",
...
def test_run_all_apply_resume_after_keyboard_interrupt_preserves_checkpoint(
    ...
    * ``KeyboardInterrupt`` propagation (not swallowed).
    ...
    monkeypatch.setattr("osm_polygon_website_tag.workflow._upload_public_shard", upload_shard)
    monkeypatch.setattr(
        "osm_polygon_website_tag.workflow.publish_to_hf",
        lambda run_dir, **_kwargs: final_uploads.append(Path(run_dir)),
    )
