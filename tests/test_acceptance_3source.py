...
from osm_polygon_website_tag.analyze import analyze_results
from osm_polygon_website_tag.card import build_card
from osm_polygon_website_tag.card_stats import compute_card_stats
from osm_polygon_website_tag.extraction import extract_pbf
from osm_polygon_website_tag.finalize import finalize_run
from osm_polygon_website_tag.run_state import initialise_run
from osm_polygon_website_tag.verify import verify_results
...
def test_acceptance_three_sources_end_to_end(make_pbf, tmp_path: Path) -> None:
    ...
    from osm_polygon_website_tag.text_schema import count_words
    ...
    for shard in sorted((run_dir / "polygons").glob("*.parquet")):
        ...
        from osm_polygon_website_tag.run_state import hash_shard, update_public_shard_metadata
    ...
    assert "Source PBFs processed: 3" in text
    assert "ODbL" in text
...
def test_acceptance_hole_reduces_area(make_pbf, tmp_path: Path) -> None:
    ...
    assert len(rows) == 1
    from osm_polygon_website_tag.geometry import compute_polygon_area_m2
