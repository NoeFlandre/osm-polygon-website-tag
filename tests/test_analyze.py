...
from osm_polygon_website_tag.analyze import (
    ANALYSIS_FILES,
    analyze_results,
)
...
""",
    ...
    from osm_polygon_website_tag.comparison_schema import (
...
def test_analysis_promotion_failure_preserves_previous_bundle(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "osm_polygon_website_tag.analyze.atomic_promote_bundle",
        fail_promotion,
    )
...
def test_analyze_results_swallows_duckdb_with_low_memory(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("osm_polygon_website_tag.duckdb_engine.DEFAULT_MEMORY_LIMIT", "32MB")
    run_dir = _make_minimal_run(tmp_path)
    _write_comparison_shard(
        run_dir,
        stem="monaco-latest",
